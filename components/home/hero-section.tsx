...
const imageSlides = [
  "/abilityx/Extra Pictures/AbilityX-4.jpg",
  "/abilityx/Extra Pictures/AbilityX-435.jpg",
  "/gallery/Networking.jpg",
  "/Team Photo.jpg",
];

const actions = [
  { href: "#ask", label: "Partner With Us", variant: "primary" as const },
  { href: "/events/1.0", label: "Watch the 1.0 Recap", variant: "outline" as const },
];
      ...
      {imageSlides.map((imageSrc, index) => (
        <div
          ...
          style={{
            ...
            animation: activeSlide === index ? "ken-burns 8s ease-in-out forwards" : undefined,
          }}
          role={index === 0 ? "img" : undefined}
          aria-label={index === 0 ? "AbilityX hero banner photography." : undefined}
        />
      ))}
      <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(31,31,31,0.6) 0%, rgba(13,13,13,0.85) 100%)" }} />
        ...
        <div className="flex min-h-[760px] items-end px-5 pb-[72px] pt-16 md:px-8 lg:px-20">
          <div className="max-w-[920px]">
            <div className="flex items-center gap-3">
              ...
              <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-brand-red">AbilityX 2.0</p>
            </div>
            <div>
              <h1 className="display-heading max-w-[10ch] text-[38px] leading-none md:text-[64px] lg:text-[80px]">
                The Movement Returns This November
              </h1>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-5">
              {actions.map((action) => (
                <PillButton key={action.label} href={action.href} variant={action.variant} className="px-6 py-3.5">
                  {action.label}
                </PillButton>
              ))}
            </div>
